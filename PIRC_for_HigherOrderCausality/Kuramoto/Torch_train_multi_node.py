...
def worker(node_id, gpu_id, args_dict, data_np):
    ...
    try:
        ...
        node_dir = optuna_dir / f"N_train_{args_dict.get('N_train')}_N_test_{args_dict.get('N_test')}_Node_num_{args_dict.get('node_num')}_PairStrength_{args_dict.get('Pair_strength')}_TriStrength_{args_dict.get('Tri_strength')}"
        ...
        study = optuna.create_study(
            storage=storage,
            study_name=f"node_{node_id}_N_train_{args_dict.get('N_train')}_N_test_{args_dict.get('N_test')}_study",
            direction="minimize",
        ...
        best_params['N_train'] = args_dict.get("N_train")
        best_params['out_dim'] = args_dict.get("in_dim")
        # best_out_dim = best_params.get("out_dim", None)
        best_Normalization = best_params.get("Normalization", 1)
        param_file = f"Parameters/{today}/N_train_{args_dict.get('N_train')}_N_test_{args_dict.get('N_test')}_NodeNum_{args_dict.get('node_num')}_node_{node_id}_PairStrength_{args_dict.get('Pair_strength')}_TriStrength_{args_dict.get('Tri_strength')}_Normalization_{best_Normalization}_block_dim{args_dict.get('block_dim')}.pkl"
        ...
        if device.type == "cuda":
            try:
                ...
                pass
        return {"node_id": node_id, "gpu_id": gpu_id, "best_value": study.best_value, "best_params": best_params, "status": "ok"}
    except Exception as e:
        # Return error info to caller
        return {"node_id": node_id, "gpu_id": gpu_id, "error": str(e), "status": "error"}
...
def objective(trial, X, in_dim, N_start, N_washout, N_test, N_rep, block_dim, ExpandNodes, use_Sin, N_train, device):
    ...
    n_units = trial.suggest_categorical('n_units', [1000,3000])
...
if __name__ == '__main__':
    ...
    if data_file.exists():
        ...
        a2, a3, data = pickle.load(open(data_file, "rb"))
        with open(data_file, "rb") as f:
            a2, a3, data = pickle.load(f)
    else:
        # generate_kuramoto_data must be available
        print("data not found, generating new data.")
        a2, a3, data = generate_kuramoto_data(n=args.node_num, dt=0.01, steps=100000, Pair_strength=args.Pair_strength,
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as exe:
        ...
        for node_id in node_ids:
            ...
            fut = exe.submit(worker, node_id, gpu_id, args_dict, data_np)
            futures[fut] = (node_id, gpu_id)

        for fut in as_completed(futures):
            node_id, gpu_id = futures[fut]
            try:
                res = fut.result()
            except Exception as e:
                res = {"node_id": node_id, "gpu_id": gpu_id, "error": str(e), "status": "error"}
            results.append(res)
            if res.get("status") == "ok":
                print(f"[Main] Finished node {res['node_id']} on GPU {res['gpu_id']}: best_value={res['best_value']}")
            else:
                print(f"[Main] Node {node_id} failed on GPU {gpu_id}: {res.get('error')}")
