...
class PIRC_Torch:
    ...
    def dataProcess(self, X, option=1):
        ...
        X_aug2=torch.cat([X[:, :1], X], dim=1)
        if  self.block_dim == 2:
            # X = torch.cat([torch.ones((rows, 2), device=self.device), sin_cos_theta(X)], dim=1) @ self.Structured_W
            # X = torch.cat([torch.full((rows, 2), math.pi / 2, device=self.device), sin_cos_theta(X)], dim=1) @ self.Structured_W
            X = sin_cos_theta(X_aug2) @ self.Structured_W
    ...
    def step(self, r_pre, x, A=None):
        ...
        r_new=(x * self.sigma_in) @ self.Win.T
        r_post = self.alpha * torch.tanh(r_new + r_pre @ self.Wres.T) + (1 - self.alpha) * r_pre
        # if option == 0:
        #     r_post = r_post
        return r_post
    ...
    def evolve_edge_removal(self, r0, j, i, N_evo, Y_train = None, Y_test = None):
        ...
        if self.out_dim == 1:
            ...
            for k in range(1, N_evo + 1):
                R_intervened[k] = self.step(R_intervened[k - 1], Yh[k - 1], A=A_intervened_in)
            ...
            for k in range(1, N_evo + 1):
                ...
                # if self.mode == 2:
                #     R_intervened[k] = self.step(R_intervened[k - 1], Yh[k - 1], A=A_intervened_in, mode = self.mode)
                #     Yh[k] = R[k] @ self.Wout * Aout + R_intervened[k] @ self.Wout * A_intervened_out
        return Yh[1:].cpu().numpy()
